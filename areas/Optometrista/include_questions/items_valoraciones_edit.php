...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($mysqli->query($consulta) === TRUE) {
        ...
        $consulta2 = "UPDATE valoracionopto SET Valoracion='$valoracion', Diagnostico='$diagnostico', Notas='$nota' WHERE ID_Registro = '$id_registro'";

        if ($mysqli->query($consulta2) === TRUE) {

            echo '<script language="javascript"> alert("Valoraciones editadas correctamente"); window.history.back(); </script>';
            exit();
        } else {
