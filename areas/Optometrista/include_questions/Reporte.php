<?php
    ...
    //signos vitales
    $sql = "SELECT Valoracion, Diagnostico, Notas FROM  valoracionopto WHERE ID_Registro='$ID'";
    ...
    if ($resultado->num_rows > 0) {
        while($row = $resultado->fetch_assoc()){
            $pdf->Ln(5);
            $pdf->SetFont("Arial", "B", 12);
            $pdf->Cell(190, 5, "VALORACIONES", 0, 1, "C");
            $pdf->Ln(2);
            $pdf->SetFont("Arial", "", 10); 
            $pdf->MultiCell(201, 5, utf8_decode("VALORACIÓN: ".$row['Valoracion']), 0, "c");
            $pdf->MultiCell(201, 5, utf8_decode("DIAGNÓSTICO: ".$row['Diagnostico']), 0, "c");
