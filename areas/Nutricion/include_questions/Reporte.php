<?php
    ...
    //personales patologicos
        $sql = "SELECT Tabaquismo, Tabpasivo, Inicio, Cantidad, ExTabaquismo, Hepatico, Cardiacas, Diabeticos, Quirurgicos, Renal, Fracturas,
                Hozpitalizacion, Hipertensivos, Neurologicos, Observaciones FROM personalespat WHERE ID_Registro='$ID'";
    ...
    if ($resultado->num_rows > 0) {
        while($row = $resultado->fetch_assoc()){
            ...
            $pdf->Multicell(201, 5, utf8_decode("HOZPITALIZACIÓN: ".$row['Hozpitalizacion']), 0, "c");
            $pdf->Multicell(201, 5, utf8_decode("HIPERTENSIVOS: ".$row['Hipertensivos']), 0, "c");
    ...
    //Ginecologicos
    $sql = "SELECT FechaMasto, Resultado FROM ginecoobs WHERE ID_Registro='$ID'";
